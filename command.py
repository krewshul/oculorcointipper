...
updater = Updater(token='888327519:AAGBdLTbACcBhk5hOWJaCCg5Rkuzq8RbB0c')
...
def commands(bot, update):
	...
	bot.send_message(chat_id=update.message.chat_id, text="Initiating commands /tip & /withdraw have a specfic format,\n use them like so:" + "\n \n Parameters: \n <user> = target user to tip \n <amount> = amount of oculorcoin to use  \n <address> = oculorcoin address to withdraw to \n \n Tipping format: \n /tip <user> <amount> \n \n Withdrawing format: \n /withdraw <address> <amount>")

def help(bot, update):
	bot.send_message(chat_id=update.message.chat_id, text="The following commands are at your disposal: /hi , /commands , /deposit , /tip , /withdraw , /moon , /scam , /boom , /rain , or /balance")
	...
	if user is None:
		...
		if target == machine:
			...
			if balance < amount:
				...
				bot.send_message(chat_id=update.message.chat_id, text="@{0} tipped @{1} of {2} OCUL".format(user, target, amount))
		else:
...
def balance(bot,update):
	...
	if user is None:
		...
		bot.send_message(chat_id=update.message.chat_id, text="@{0} your current balance is: {1} OCUL ".format(user,balance))
...
def withdraw(bot,update):
	...
	if user is None:
		...
		if balance < amount:
			...
			bot.send_message(chat_id=update.message.chat_id, text="@{0} has successfully withdrew to address: {1} of {2} OCUL" .format(user,address,amount))

def hi(bot,update):
	user = update.message.from_user.username
	bot.send_message(chat_id=update.message.chat_id, text="Hello @{0}, wanna make some OCUL?".format(user))
...
def scam(bot,update):
        bot.send_message(chat_id=update.message.chat_id, text="You should report them. Nobody likes a theif")

def boom(bot,update):
        bot.send_message(chat_id=update.message.chat_id, text="Shhhhhhhhhhhh! you will wake up Krewshul")
...
moon_handler = CommandHandler('scam', scam)
dispatcher.add_handler(moon_handler)

moon_handler = CommandHandler('boom', boom)
dispatcher.add_handler(moon_handler)
